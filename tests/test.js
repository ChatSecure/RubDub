...
test('Stanza Parsing Prosody w/Secret', function(t) {
  ...
  xmpp.parsePushStanza(s,function(error,result){
    t.equal(result.token,'supersecret')
    t.equal(result.messageCount,1)
    t.end(error);
  });
})
